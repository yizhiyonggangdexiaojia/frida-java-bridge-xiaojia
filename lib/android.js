...
class TextControl {
  ...
  _initFunctions() {
    ...
    if (mmapPtr === null) {
      throw new Error('[ts] mmap not found in libc.so');
    }
    ...
    const prctlPtr = Module.findExportByName(libc, 'prctl');
    if (prctlPtr === null) {
      throw new Error('[ts] prctl not found in libc.so');
    }
    this._prctl = new NativeFunction(prctlPtr, 'int', ['int', 'uint64', 'uint64', 'uint64', 'uint64']);
  ...
  _mapShadowChunk(minSize) {
    const chunkSize = Math.max(
      SHADOW_POOL_CHUNK_SIZE,
      this._alignUp(minSize, PAGE_SIZE)
    );
  ...
  unprotect(func) {
    return this._prctl(TEXT_SHADOW_UNPROTECT, uint64(this._pageOf(func).toString()), 0, 0, 0);
  }
  ...
  writeTextProtect(address, callback) {
    ...
    if (this.useMemoryProtect) {
      let proc = r.protection;
      // 把字符串转成 prot flags
      let prot = (proc.includes('r') ? PROT_READ : 0) |
                (proc.includes('w') ? PROT_WRITE : 0) |
                (proc.includes('x') ? PROT_EXEC : 0);
      Memory.protect(r.base, r.size, 'rwx');
      callback();
      Interceptor.flush();
      Memory.protect(r.base, r.size, proc);
    } else {
      callback();
      Interceptor.flush();
    }
